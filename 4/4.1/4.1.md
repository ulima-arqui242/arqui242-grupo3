...
**Titulo:** Elección entre Replicación de Base de Datos vs Estrategia de Sharding para el Módulo de Consultas de Disponibilidad
...
2. **Sharding:**  
   - Divide la base de datos en fragmentos (shards), distribuyendo la carga entre múltiples servidores.
   - Escalabilidad Horizontal: Permite agregar más servidores para manejar mayores volúmenes de datos y consultas.
   - Complejidad de Gestión: La implementación y mantenimiento son más complicados.
   - Consistencia: Puede haber problemas de consistencia si los datos no están bien distribuidos o gestionados.
...
**Sustento:** La táctica de replicación de base de datos garantiza una mayor disponibilidad del sistema, incluso si la base de datos principal falla. Aunque el sharding es muy efectivo para manejar grandes volúmenes de datos y tráfico, introduce una complejidad adicional en la gestión y puede afectar la consistencia de los datos si no se implementa adecuadamente. La replicación de base de datos es seleccionada porque prioriza la experiencia del usuario, asegurando que siempre haya acceso a datos precisos, especialmente en momentos de alta demanda, como fines de semana y festivos. Esto minimiza el riesgo de pérdidas de reservas y mejora la reputación del servicio.
...
#### Táctica Elegida
Para llegar a los niveles de disponibilidad deseados, en este módulo se elige una táctica de degradación. A continuación los detalle de la decisión. La redundancia activa asegura una conmutación imperceptible y máxima disponibilidad
...
#### Táctica Elegida
Se elige la táctica de redundancia activa en el sistema de autenticación. 
...
1. **Redundancia Activa:**
   ...
   - **Ventajas:**
     - **Conmutación Imperceptible:** Los usuarios no experimentan tiempos de espera, mejorando la experiencia general.
     - **Alta Disponibilidad:** Minimiza el riesgo de caídas en el servicio, manteniendo la autenticación operativa en todo momento.
   - **Desventajas:**
     - **Mayor Costo de Operación:** Requiere el mantenimiento simultáneo de varios servidores activos, aumentando significativamente los gastos operativos.

2. **Redundancia Pasiva:**
   - Un servidor de autenticación secundario está en modo de espera (inactivo) y se activa cuando el servidor principal falla.
   - **Ventajas:**
     - **Menor Costo de Operación:** Solo se necesita un servidor en funcionamiento en todo momento, reduciendo costos.
     - **Implementación Sencilla:** La gestión de un único servidor activo puede simplificar el mantenimiento y la operación.
   - **Desventajas:**
     - **Tiempo de Conmutación:** Aunque el tiempo de conmutación es aceptable (menos de 5 segundos), los usuarios pueden experimentar una breve interrupción en el servicio durante este periodo.
...
**Decisión:** Se elige la **redundancia activa** para el módulo de autenticación.

**Sustento:** La redundancia activa asegura una conmutación imperceptible y máxima disponibilidad, lo que es crítico para la experiencia del usuario en un sistema de autenticación. Aunque su implementación conlleva un costo operativo más alto, la capacidad de mantener el servicio sin interrupciones justifica esta inversión, especialmente en un entorno donde la autenticación es esencial para el acceso continuo al sistema.
