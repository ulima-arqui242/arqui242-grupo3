...
## Módulo 06 - Analíticas y Reporte
### Escenario: Cambio de procesador de pagos

| **Cod Escenario** | **Atributo de Calidad** | **Estímulo** | **Fuente del Estímulo** | **Artefacto** | **Entorno** | **Respuesta** | **Medida de Respuesta** |
| --- | --- | --- | --- | --- | --- | --- | --- |
| ESC-10 | Mantenibilidad | Se recibe una solicitud de modificación en el formato de exportación de datos a otro formato además de CSV (e.g., Excel) | Equipo de desarrollo | Funcionalidad de exportación de datos | Mantenimiento del sistema | El código se adapta fácilmente para permitir múltiples formatos de exportación sin afectar la funcionalidad original | El tiempo para agregar un nuevo formato no excede 1 día |

#### Táctica Elegida
Se elige la táctica de parametrización para permitir la exportación a múltiples formatos (e.g., CSV, Excel). Esto implica diseñar el componente de exportación de manera flexible, con opciones configurables para soportar nuevos formatos sin modificar el código base.

### Documentación de la Decisión (ADR)
**Título:** Elección de Parametrización para Exportación de Datos a Múltiples Formatos

**Contexto:** La funcionalidad de exportación de datos en el módulo de analíticas debe ser flexible para soportar múltiples formatos de archivo (CSV, Excel, etc.). La parametrización permitirá realizar cambios o agregar nuevos formatos sin afectar el comportamiento del sistema ni otros componentes.

**Alternativas:**
1. **Exportación Directa en Formato Fijo:**
   - La lógica del sistema define un único formato de exportación (CSV).
   - Cualquier cambio de formato requiere modificar la lógica del sistema de exportación.
   - Alto riesgo de errores si el cambio no se gestiona adecuadamente.

2. **Parametrización:**
   - Se parametrizan los formatos de exportación, permitiendo elegir entre diferentes opciones (CSV, Excel) a través de un archivo de configuración o interfaz.
   - Agregar un nuevo formato solo requiere actualizar los parámetros de configuración, sin modificar la lógica central del sistema.
   - La interfaz del sistema se mantiene estable, lo que asegura la robustez y la facilidad de mantenimiento.

3. **Uso de un intermediario (Adaptador - Disminuir acoplamiento):**
   - Implementar un patrón adaptador que transforme los datos exportados en el formato necesario (CSV, Excel, PDF).
   - La exportación se realizaría en un formato intermedio, y luego el adaptador se encargaría de convertirlo al formato solicitado.
   - Este enfoque asegura que los cambios en los formatos de exportación no afecten la lógica central, pero añade una capa adicional de complejidad en la arquitectura.

**Criterios de Elección:**
   - Flexibilidad para soportar nuevos formatos de exportación.
   - Tiempo de implementación y facilidad de prueba.
   - Mayor mantenibilidad y adaptabilidad del sistema a futuro.
   - Balance entre simplicidad y escalabilidad.


**Decisión:** Se elige la parametrización como estrategia para soportar la exportación a múltiples formatos de manera flexible.

**Sustento:** La parametrización permite que el sistema de exportación de datos pueda agregar nuevos formatos de archivo sin alterar la lógica principal. Esta estrategia mejora la mantenibilidad y flexibilidad del sistema, lo que facilita futuras modificaciones y asegura que el módulo de analíticas siga funcionando correctamente sin interrupciones o errores.
