# 3.2. Modelo de Coordinación

## Servicios Sincrónicos y Asincrónicos

### Servicios Sincrónicos
Un servicio sincrónico es aquel que responde inmediatamente a las solicitudes, utilizando típicamente HTTP, donde el cliente espera una respuesta inmediata antes de continuar.

- **Reservation Service (CRUD)**: Este servicio maneja las reservas (crear, leer, actualizar y eliminar). Las operaciones CRUD suelen ser sincrónicas, porque el cliente necesita una respuesta inmediata al solicitar o modificar información de las reservas.

- **Auth Service**: El servicio de autenticación es sincrónico ya que se requiere verificar la identidad del usuario en tiempo real antes de permitirle el acceso a otros recursos.

- **Admin Service**: Como este servicio gestiona la adición de servicios al spa, lo cual podría requerir una respuesta inmediata, también podría considerarse un servicio sincrónico.
  
- **User Service**: Dado que este servicio permite cambiar la foto del usuario y otras configuraciones , se trata de una operaciones sincrónicas que deben completarse en tiempo real para mejorar la experiencia del usuario.
  
### Servicios Asincrónicos
Los servicios asincrónicos permiten enviar solicitudes sin esperar una respuesta inmediata. Generalmente, se manejan mediante colas de mensajes o notificaciones.

- **Notifications Service**: Este servicio usa **Amazon SNS** para manejar notificaciones push asincrónicas. Las notificaciones se envían sin que el cliente necesite esperar una respuesta inmediata.

- **Payments Service (con Stripe)**: Aunque el proceso de pago puede comenzar de forma sincrónica, la confirmación del pago se maneja de manera asincrónica, ya que las transacciones pueden demorar tiempo en completarse.

- **Analytics Service**: Los servicios de análisis recopilan y procesan datos en segundo plano, por lo que este servicio es típicamente asincrónico.

- **Offers Service**: El manejo de ofertas puede ser asincrónico, ya que las ofertas suelen actualizarse o procesarse en lotes y no requieren una respuesta inmediata.


## ADR 1: Arquitectura Stateful vs. Arquitectura Stateless

### Contexto:

La plataforma de **InStudio** debe gestionar interacciones relacionadas con la autenticación y autorización de usuarios, en módulos como itinerarios personalizados, reseñas y la seguridad de APIs. La arquitectura debe permitir una escalabilidad eficiente, ser tolerante a fallos y fácil de mantener. Además, es fundamental asegurar una buena integración con tecnologías de autenticación como JSON Web Tokens (JWT), que ya se están considerando en el proyecto.

### Alternativas:

#### Arquitectura Stateful

- Los servidores retienen el estado entre solicitudes del usuario, lo que permite mantener el contexto de la sesión durante toda la interacción.
- Proporciona una experiencia más fluida cuando el contexto del usuario es crucial y se requiere una interacción continua.
- Requiere sincronización del estado entre servidores o el uso de mecanismos como "sticky sessions", lo que complica la escalabilidad.
- Es más vulnerable a fallos, ya que la pérdida de un servidor que almacena el estado puede afectar negativamente la experiencia del usuario.

#### Arquitectura Stateless

- No se conserva el estado en el servidor entre solicitudes; cada interacción es independiente y lleva la información necesaria, como un token JWT.
- Facilita una escalabilidad más sencilla, ya que no hay necesidad de sincronizar estados ni mantener sesiones activas en los servidores.
- Más resiliente, ya que los fallos en un servidor no afectan la experiencia del usuario, y las solicitudes pueden distribuirse fácilmente entre múltiples servidores.
- Ideal para microservicios y sistemas distribuidos donde las interacciones rápidas y sin dependencia del estado previo son necesarias.

### Criterios de Elección:

1. Facilidad de escalabilidad y capacidad para manejar cargas de trabajo crecientes sin aumentar la complejidad.
2. Tolerancia a fallos y capacidad del sistema para continuar operando ante la caída de servidores.
3. Simplicidad en la gestión e implementación del estado de los usuarios.
4. Compatibilidad con las tecnologías de autenticación actuales (como JWT), que permiten una validación eficiente de usuarios en cada solicitud.

### Decisión:

Se opta por una arquitectura **stateless**.

### Sustento:

La arquitectura stateless es la más adecuada para **InStudio**, ya que ofrece alta escalabilidad y mayor resiliencia, aspectos clave para gestionar un volumen creciente de usuarios. Además, se integra de manera eficiente con JWT, lo que facilita la autenticación de usuarios sin depender del estado en el servidor. Esta decisión reduce la complejidad operativa y elimina los riesgos asociados a la sincronización de sesiones, garantizando un mejor rendimiento y una plataforma más tolerante a fallos.


## ADR 2: API Gateway vs. Llamadas Directas

### Contexto:

La plataforma de **InStudio** cuenta con varios módulos, como autenticación, itinerarios personalizados, reseñas y la seguridad de APIs. Estos módulos deben interactuar tanto entre sí como con servicios externos. La elección de cómo orquestar esta interacción afectará la escalabilidad, seguridad y mantenibilidad del sistema. El sistema ya maneja autenticación con JWT y posiblemente OAuth 2.0, lo que también influye en cómo se estructuran las comunicaciones entre los servicios.

### Alternativas:

#### API Gateway

- Un **API Gateway** actúa como un intermediario único entre el frontend y los microservicios, gestionando todas las solicitudes de los clientes hacia los diferentes servicios internos.
- Facilita la autenticación centralizada, el control de tráfico, y permite agregar políticas de seguridad y limitación de tasas.
- Simplifica el frontend al abstraer la complejidad de las múltiples llamadas a servicios, consolidándolas en una única entrada.
- Puede introducir un "single point of failure" y agregar latencia si no se optimiza adecuadamente, pero reduce la carga en los servicios internos.

#### Llamadas Directas entre Servicios

- Cada módulo o servicio se comunica directamente con los demás, sin intermediarios, lo que permite una interacción más sencilla.
- Evita la introducción de un nuevo componente en la infraestructura, reduciendo la complejidad inicial de implementación.
- Proporciona menos control centralizado sobre la seguridad, ya que cada servicio debe manejar su propia autenticación, limitación de tasas y enrutamiento.
- A medida que la plataforma crece, la cantidad de interacciones entre servicios puede incrementar significativamente la complejidad y el riesgo de fallos, dificultando su mantenimiento.

### Criterios de Elección:

1. Capacidad de escalabilidad y manejo de grandes volúmenes de tráfico.
2. Control centralizado sobre políticas de seguridad y autenticación.
3. Facilidad de mantenimiento a largo plazo, considerando el crecimiento de la plataforma y el número de servicios.
4. Latencia y rendimiento en la interacción entre los módulos.

### Decisión:

Se decide implementar un **API Gateway**.

### Sustento:

El **API Gateway** es la opción más adecuada para la plataforma de **InStudio**, ya que consta de varios módulos que necesitan interactuar constantemente. La centralización de la autenticación y la gestión de tráfico en el gateway proporciona un mayor control sobre la seguridad, lo cual es fundamental para manejar solicitudes autenticadas con JWT y OAuth 2.0. Aunque puede agregar complejidad y algo de latencia, el **API Gateway** mejora la escalabilidad, simplifica el mantenimiento y permite una administración más eficiente de las interacciones entre servicios, lo cual es clave para un sistema en crecimiento.

## ADR 3: Comunicación entre Microservicios - Kafka, REST o gRPC

### Contexto:

La plataforma de **InStudio** está compuesta por varios microservicios, cada uno encargado de funciones específicas, como autenticación, gestión de usuarios, pagos y notificaciones. Estos servicios necesitan comunicarse entre sí para garantizar una operación fluida del sistema. La elección del protocolo de comunicación afectará la latencia, escalabilidad, resiliencia y facilidad de mantenimiento del sistema. Se debe considerar el volumen de tráfico esperado, la necesidad de comunicación síncrona o asíncrona, y la posibilidad de manejar grandes volúmenes de datos y eventos.

### Alternativas:

#### Kafka (Mensajería Asíncrona)

- **Kafka** es una plataforma de mensajería distribuida basada en el modelo de pub/sub, que permite la transmisión de eventos en tiempo real entre microservicios.
- Ideal para la comunicación asíncrona, donde los servicios no necesitan esperar respuestas inmediatas y pueden procesar eventos a medida que llegan.
- Permite una alta escalabilidad, ya que puede manejar grandes volúmenes de eventos y datos de manera eficiente.
- Aporta resiliencia, ya que si un servicio está temporalmente fuera de línea, puede consumir los mensajes cuando esté disponible.
- Puede ser complejo de implementar y gestionar, ya que requiere configurar brokers, particiones, y consumidores/proveedores, lo que introduce complejidad en la infraestructura.

#### REST (Comunicación Síncrona)

- **REST** es un protocolo basado en HTTP que permite la comunicación directa y síncrona entre microservicios mediante peticiones y respuestas.
- Sencillo de implementar y compatible con la mayoría de lenguajes y frameworks, lo que lo hace ideal para la interoperabilidad.
- Adecuado para casos donde se requiere una respuesta inmediata, como consultas de datos o confirmación de transacciones.
- Sin embargo, la naturaleza síncrona de REST lo hace menos adecuado para operaciones de larga duración o alta carga, ya que un servicio puede quedarse esperando la respuesta de otro, afectando la latencia y el rendimiento del sistema.
- Menos eficiente para manejar grandes volúmenes de eventos en tiempo real.

#### gRPC (Comunicación Síncrona y Eficiente)

- **gRPC** es un protocolo de comunicación de alto rendimiento desarrollado por Google, basado en HTTP/2 y Protobuf, que permite la comunicación síncrona y asíncrona entre microservicios.
- Soporta la comunicación bidireccional y transmisión de datos eficiente, lo que lo hace adecuado para servicios que requieren baja latencia y alto rendimiento.
- Es ideal para sistemas distribuidos de gran escala donde se requiere comunicación eficiente y rápida.
- Comparado con REST, gRPC es más eficiente en el uso de ancho de banda, pero su implementación puede ser más compleja debido a la necesidad de definir esquemas (Protobuf) y la compatibilidad con HTTP/2.
- Adecuado para interacciones de alta frecuencia y bajo tiempo de respuesta, como comunicación entre servicios críticos del backend.

### Criterios de Elección:

1. Naturaleza de la comunicación (síncrona vs. asíncrona).
2. Volumen de tráfico y escalabilidad requerida.
3. Resiliencia del sistema ante fallos o interrupciones.
4. Facilidad de implementación y mantenimiento a largo plazo.
5. Necesidad de interoperabilidad y rendimiento en tiempo real.

### Decisión:

Se elige implementar **Kafka** para la comunicación entre microservicios

### Sustento:

Para la plataforma de **InStudio**, **Kafka** es la mejor opción para manejar la comunicación asíncrona entre microservicios que producen y consumen eventos, como notificaciones, pagos y analíticas. Kafka permite una alta escalabilidad y resiliencia, lo que es crucial para un sistema que puede manejar grandes volúmenes de datos y necesita asegurar que los mensajes no se pierdan si un servicio está temporalmente inactivo.

**GRPC** , aunruqe eficiente, presenta una dificultad para el equipo por el manejo y falta de experiencia en s implementación.

**REST**, aunque fácil de implementar, se descarta para la mayoría de las interacciones críticas, ya que su naturaleza síncrona y basada en texto puede generar latencias innecesarias y no es lo suficientemente eficiente para manejar grandes volúmenes de datos o solicitudes frecuentes entre microservicios.


## ADR 4: Exposición de APIs para la Plataforma Web

### Contexto:

La plataforma de **InStudio** requiere exponer una serie de APIs para que la aplicación web pueda interactuar con sus microservicios. Estas APIs serán responsables de manejar diversas funciones, como autenticación, gestión de usuarios, itinerarios personalizados, y reseñas. Es crucial definir cómo se expondrán estas APIs para garantizar la seguridad, facilidad de uso y mantenibilidad del sistema.

### Alternativas:

#### RESTful APIs

- **RESTful APIs** son un enfoque popular y ampliamente adoptado para la construcción de APIs web, basadas en el protocolo HTTP.
- Proporcionan un conjunto de convenciones y principios que facilitan la interacción entre el cliente y los microservicios a través de métodos HTTP (GET, POST, PUT, DELETE).
- Sencillas de implementar y entender, RESTful APIs permiten la integración con diversas plataformas y lenguajes de programación.
- Se benefician de la caché HTTP, lo que puede mejorar el rendimiento de las solicitudes.
- Sin embargo, pueden presentar limitaciones en términos de eficiencia y rendimiento para comunicaciones de alta frecuencia, debido a la sobrecarga del protocolo y la necesidad de establecer una conexión en cada solicitud.

#### GraphQL

- **GraphQL** es un lenguaje de consulta para APIs que permite a los clientes especificar exactamente qué datos necesitan.
- Ofrece flexibilidad al permitir a los desarrolladores realizar consultas complejas en una sola solicitud, lo que puede reducir la cantidad de solicitudes necesarias.
- Facilita la evolución de las APIs, ya que los clientes pueden solicitar solo los campos que necesitan, minimizando el riesgo de romper la compatibilidad.
- Sin embargo, puede ser más complejo de implementar y requerir más trabajo en el lado del servidor para gestionar las consultas y sus resoluciones.

#### gRPC

- **gRPC** es un framework de comunicación de alto rendimiento que utiliza HTTP/2 para la transmisión de datos.
- Permite la definición de APIs a través de Protobuf, ofreciendo una interfaz más estricta y eficiente para la comunicación entre microservicios.
- Ideal para sistemas donde se requiere alta eficiencia y bajo tiempo de respuesta.
- Sin embargo, puede ser menos accesible para los desarrolladores que no están familiarizados con Protobuf y puede requerir más tiempo de desarrollo inicial.

### Criterios de Elección:

1. Facilidad de implementación y adopción por parte del equipo de desarrollo.
2. Flexibilidad y capacidad de evolucionar la API sin romper la compatibilidad.
3. Eficiencia en la transmisión de datos y rendimiento bajo cargas altas.
4. Facilidad de integración con clientes web y móviles.
5. Consideraciones de seguridad y autenticación.

### Decisión:

Se decide exponer las APIs utilizando **RESTful APIs**.

### Sustento:

La elección de **RESTful APIs** para exponer las funcionalidades de la plataforma de **InStudio** se basa en su simplicidad y facilidad de implementación. Este enfoque es ampliamente conocido y utilizado, lo que facilita la integración y colaboración entre los equipos de desarrollo y los clientes. 

Además, RESTful APIs permiten manejar diversas funcionalidades con métodos HTTP, garantizando una comunicación clara y eficiente entre la aplicación web y los microservicios subyacentes. Aunque hay alternativas como GraphQL y gRPC, RESTful APIs proporcionan la combinación óptima de accesibilidad, flexibilidad y facilidad de mantenimiento, lo que es esencial para el crecimiento y evolución de la plataforma. 

Finalmente, se implementarán medidas de seguridad adecuadas, como autenticación con JWT, para proteger las APIs expuestas y garantizar la seguridad de los datos de los usuarios.



## ADR 5: Elección entre Apache Kafka vs Alternativas de Mensajería Asíncrona

### Contexto
Para la comunicación asíncrona entre los microservicios de InStudio, se requiere seleccionar un sistema de mensajería que permita una gestión eficiente de eventos y solicitudes concurrentes. Es crucial evaluar Apache Kafka frente a otras soluciones de mensajería.

### Alternativas

#### **5.1 Apache Kafka**

**Ventajas**:
- Alta Escalabilidad: Soporta un alto volumen de mensajes y eventos, permitiendo una escalabilidad horizontal eficaz.
- Desacoplamiento: Facilita la comunicación asíncrona entre microservicios, desacoplando productores y consumidores de eventos.
- Persistencia y Durabilidad: Ofrece almacenamiento duradero de mensajes, permitiendo la recuperación y reprocesamiento de eventos.

**Desventajas**:
- Complejidad: La configuración y mantenimiento de Kafka puede ser complejo.
- Requerimientos de Recursos: Necesita recursos adicionales para operar y mantener el clúster de Kafka.


#### **5.2 RabbitMQ**

**Ventajas**:
- Facilidad de Uso: Más fácil de configurar y administrar en comparación con Kafka.
- Flexibilidad de Enrutamiento: Proporciona capacidades avanzadas de enrutamiento y patrones de consumo de mensajes.

**Desventajas**:
- Escalabilidad Limitada: Menos eficiente en el manejo de grandes volúmenes de mensajes y alta concurrencia.
- Persistencia: La persistencia de mensajes es menos robusta en comparación con Kafka.

    
### Criterios de Elección

- Escalabilidad y rendimiento.
- Complejidad en la configuración y mantenimiento.
- Flexibilidad en el manejo de mensajes y eventos.
- Costos operativos y recursos necesarios.
  
### Decisión

Se elige Apache Kafka como sistema de mensajería asíncrona.

### Sustento

Apache Kafka es la mejor opción porque maneja grandes volúmenes de mensajes de manera eficiente y asegura que los mensajes se conserven y puedan recuperarse fácilmente. Aunque es más complejo y requiere más recursos, su capacidad de escalar y gestionar eventos asíncronos supera a otras opciones como RabbitMQ. Kafka es más adecuado para una plataforma en crecimiento como InStudio.



## ADR 6: Elección de framework de frontend para el desarrollo

### Contexto
El equipo de desarrollo está considerando qué framework o biblioteca utilizar para el frontend de la aplicación. Los miembros del equipo tienen experiencia previa con React y están buscando una opción que les permita trabajar de manera eficiente y aprovechar su experiencia existente.

### Alternativas

#### **6.1 React**

**Ventajas:**
- Experiencia del Equipo: El equipo ya tiene experiencia con React, lo que puede acelerar el desarrollo y reducir la curva de aprendizaje.
- Flexibilidad: React ofrece una gran flexibilidad y un rico ecosistema de herramientas y librerías.
- Componentización: Facilita el desarrollo mediante componentes reutilizables y una arquitectura basada en componentes.
- Desempeño: Utiliza un DOM virtual para mejorar el rendimiento de las actualizaciones de la interfaz de usuario.

**Desventajas:**
- **Requiere Configuración Adicional**: Puede necesitar más configuración y herramientas adicionales para manejar el enrutamiento, el estado, etc.


#### **6.2 Angular**

**Ventajas:**
- Todo en Uno: Incluye muchas características listas para usar, como enrutamiento, manejo de formularios y herramientas de pruebas.
- Estructura Rigurosa: Proporciona una estructura más rígida, que puede ser beneficiosa para grandes equipos o proyectos complejos.

**Desventajas:**
- Curva de Aprendizaje: Más compleja para aprender en comparación con React, especialmente si el equipo no está familiarizado.
- Rigidez: Menos flexible que React, lo que puede limitar la libertad en la implementación de soluciones personalizadas.

    
### Criterios de Elección

- Experiencia del equipo.
- Flexibilidad y facilidad de uso.
- Requerimientos de características y herramientas adicionales.
  
### Decisión
Se elige React.

### Sustento
Dado que el equipo ya tiene experiencia con React, esto permitirá un desarrollo más rápido. React ofrece la flexibilidad y el ecosistema necesario para construir la interfaz de usuario, mientras que la experiencia previa del equipo ayudará a evitar la curva de aprendizaje asociada con Angular.
